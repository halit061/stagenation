/*
  # Fix ticket_seats.ticket_id assignment in create_seat_order_pending

  1. Bug Fix
    - `ticket_id` was incorrectly set to `v_order_id` instead of `v_ticket_id`
    - This fix sets `ticket_id` to the correct ticket record ID

  2. No data loss
    - The `order_id` column still correctly references the order
    - Existing records will be updated to reference the correct ticket
*/
