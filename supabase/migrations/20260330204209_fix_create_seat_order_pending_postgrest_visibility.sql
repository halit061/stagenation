/*
  # Fix create_seat_order_pending PostgREST visibility

  PostgREST returns 404 for this RPC function despite it existing in the database.
  This migration drops and recreates the function to force PostgREST to pick it up,
  then sends a schema reload notification.

  1. Recreated Functions
    - `create_seat_order_pending` - Creates a pending seat order with all ticket seats
  2. Security
    - SECURITY DEFINER with search_path = public
    - EXECUTE granted to authenticated and anon roles
*/

DROP FUNCTION IF EXISTS public.create_seat_order_pending(uuid, text, text, text, text, numeric, numeric, numeric, text, text, text, uuid[], numeric[], uuid);
...
CREATE OR REPLACE FUNCTION public.create_seat_order_pending(
  ...
  p_seat_ids uuid[] DEFAULT NULL::uuid[],
  p_seat_prices numeric[] DEFAULT NULL::numeric[],
...
GRANT EXECUTE ON FUNCTION public.create_seat_order_pending(uuid, text, text, text, text, numeric, numeric, numeric, text, text, text, uuid[], numeric[], uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.create_seat_order_pending(uuid, text, text, text, text, numeric, numeric, numeric, text, text, text, uuid[], numeric[], uuid) TO anon;
GRANT EXECUTE ON FUNCTION public.create_seat_order_pending(uuid, text, text, text, text, numeric, numeric, numeric, text, text, text, uuid[], numeric[], uuid) TO service_role;
