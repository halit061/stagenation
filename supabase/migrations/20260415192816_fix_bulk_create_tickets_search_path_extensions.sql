/*
  # Fix bulk_create_tickets search path

  1. Problem
    - The `bulk_create_tickets` function uses `gen_random_bytes()` from pgcrypto
    - pgcrypto lives in the `extensions` schema
    - The function's search_path only includes `public`, causing it to fail

  2. Fix
    - Add `extensions` to the search_path so `gen_random_bytes` is resolvable
*/
