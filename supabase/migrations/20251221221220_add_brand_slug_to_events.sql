/*
  # Add brand_slug field to events with auto-generation
  
  1. Changes
    - Add brand_slug column to events table (unique, indexed)
...
DECLARE
  event_record RECORD;
BEGIN
  FOR event_record IN SELECT id, name, start_date FROM events WHERE brand_slug IS NULL ORDER BY start_date
  LOOP
    UPDATE events
...
-- Make brand_slug NOT NULL and UNIQUE after backfill
ALTER TABLE events ALTER COLUMN brand_slug SET NOT NULL;
