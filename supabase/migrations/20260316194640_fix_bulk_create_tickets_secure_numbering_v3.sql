/*
  # Fix bulk_create_tickets - Secure ticket numbering

  1. Changes
    - Drop both existing overloads of bulk_create_tickets
    - Remove timestamp from ticket numbers (security: prevents timing attacks)
    - Remove sequential index from ticket numbers (security: prevents enumeration)
    - Use first 3 letters of ticket type name as prefix instead of hardcoded 'TKT'
    - Each ticket gets a fully random 12-char hex identifier
    - Format: PREFIX-RANDOMCHARS (e.g. EAR-A3B7C9D2E1F4)

  2. Security improvements
    - No timestamps exposed in public-facing identifiers
    - No sequential numbering that could be enumerated
    - Cryptographically random identifiers via gen_random_bytes
    - Added SET search_path = public for SECURITY DEFINER safety
*/

DROP FUNCTION IF EXISTS bulk_create_tickets(uuid, uuid, integer);
DROP FUNCTION IF EXISTS bulk_create_tickets(uuid, uuid, integer, text, text);
