...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { category_id, name, price, sku, is_active, image_url } = body;

    if (!category_id || !name || price === undefined) {
      return new Response(
        JSON.stringify({ error: 'category_id, name, and price are required' }),
        {
          status: 400,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        }
    ...
    const { data: drink, error: insertError } = await serviceSupabase
      .from('drinks')
      .insert({
        category_id,
        name,
