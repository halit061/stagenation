...
export function SeatConfirmation({ eventId, orderId, onNavigate }: Props) {
      ...
      try {
        ...
        const seatInfos: SeatInfo[] = ticketSeats.map((ts: any) => {
          ...
          return {
            ...
            section_name: section?.name || 'Onbekend',
        ...
        if (!cancelled) {
          setError(err.message || 'Kon bestelling niet laden');
          setLoading(false);
        }
      }
    }
  ...
  const handleShare = useCallback(async () => {
    ...
    const shareData = {
      ...
      text: `Check dit event: ${eventInfo.name}`,
  ...
  const formattedDate = useMemo(() => {
    ...
    return new Date(eventInfo.start_date).toLocaleDateString('nl-NL', {
      weekday: 'short',
      day: 'numeric',
      month: 'long',
      year: 'numeric',
    ...
    return (
      <div className="min-h-screen bg-slate-950 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-slate-400 text-sm">Bestelling laden...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-slate-950 flex items-center justify-center p-4">
        <div className="text-center max-w-md">
          <h1 className="text-xl font-bold text-white mb-2">Bestelling niet gevonden</h1>
          <p className="text-slate-400 mb-6">{error || 'Deze bestelling bestaat niet of is niet toegankelijk.'}</p>
          <button
    ...
    <div className="min-h-screen bg-slate-950 print:bg-white">
      <header className="bg-slate-900/80 backdrop-blur border-b border-slate-800 px-4 py-3 print:hidden">
        <div className="max-w-3xl mx-auto flex items-center gap-3">
          ...
          <h1 className="text-white font-bold text-base">Bevestiging</h1>
        </div>
      </header>
      ...
      <main className="max-w-3xl mx-auto px-4 py-8 space-y-6">
        <div className={`text-center transition-all duration-500 ${animReady ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
          ...
          <h2 className="text-2xl font-bold text-white">Bestelling Geplaatst!</h2>
          <p className="text-slate-400 mt-1">
            Bedankt voor je bestelling{firstName ? `, ${firstName}` : ''}!
          </p>
        </div>

        <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 text-center">
          <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1">Bestelnummer</p>
          ...
          {copied && <p className="text-emerald-400 text-xs mt-1">Bestelnummer gekopieerd</p>}
        </div>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden">
          <div className="px-5 py-3 border-b border-slate-800">
            <h3 className="text-white font-semibold text-sm">Stoelen Overzicht</h3>
          </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="text-slate-400 text-xs border-b border-slate-800">
                  <th className="text-left px-5 py-2 font-medium">Sectie</th>
                  <th className="text-left px-3 py-2 font-medium">Rij</th>
                  <th className="text-left px-3 py-2 font-medium">Stoel</th>
                  <th className="text-right px-5 py-2 font-medium">Prijs</th>
                </tr>
              </thead>
              <tbody>
                {seats.map(seat => (
                  ...
                  >
                    <td className="px-5 py-2.5">
                      <div className="flex items-center gap-2">
                        <div className="w-2.5 h-2.5 rounded-full flex-shrink-0" style={{ backgroundColor: seat.section_color }} />
            ...
            {Object.entries(seatsBySection).map(([sectionName, sectionSeats]) => (
              <div key={sectionName} className="space-y-1">
                <div className="flex items-center gap-2 mb-1">
                  <div className="w-2 h-2 rounded-full" style={{ backgroundColor: sectionSeats[0].section_color }} />
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 space-y-3">
          <div className="flex items-center gap-2 text-slate-300 text-sm">
            ...
            <span>Een bevestiging is verstuurd naar <strong className="text-white">{order.payer_email}</strong></span>
          </div>
          <p className="text-slate-500 text-xs leading-relaxed">
            Bewaar je bestelnummer. Je hebt dit nodig bij de ingang.
          </p>
        </div>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h3 className="text-white font-semibold text-base">Je Tickets</h3>
          </div>
          <p className="text-slate-400 text-sm mb-4">Je tickets worden binnenkort beschikbaar om te downloaden.</p>
        ...
        <div className="flex flex-col sm:flex-row gap-3 print:hidden">
          <button
            onClick={() => onNavigate('home')}
            className="flex-1 py-3 border border-blue-500/50 text-blue-400 hover:bg-blue-500/10 font-medium rounded-xl transition-colors text-sm"
          >
            Terug naar Events
          </button>
          <button
            onClick={() => onNavigate(`seat-picker?event=${eventId}`)}
            className="flex-1 py-3 bg-emerald-600 hover:bg-emerald-500 text-white font-semibold rounded-xl transition-colors text-sm"
          >
            Nog meer tickets kopen
          </button>
          <button
            onClick={handleShare}
            className="flex-1 py-3 border border-slate-700 text-slate-300 hover:bg-slate-800 font-medium rounded-xl transition-colors text-sm flex items-center justify-center gap-2"
          >
